...
public class DefaultChatClient implements ChatClient {
	...
	public static class DefaultCallResponseSpec implements CallResponseSpec {
		...
		/**
		 * changed by leo ,getReasonerContent from  AssistantMessage
		 * @param chatResponse
		 * @return
		 */
		@Nullable
		private static String getReasonerFromChatResponse(@Nullable ChatResponse chatResponse) {
			return Optional.ofNullable(chatResponse)
					.map(ChatResponse::getResult)
					.map(Generation::getOutput)
					.map(AssistantMessage::getReasonerContent)
					.orElse(null);
		}
		...
		/**
		 * //changed by leo ,this content contians content and reasoner
		 * @return
		 */
		@Override
		@Nullable
	...
	public static class DefaultStreamResponseSpec implements StreamResponseSpec {
		...
		public Flux<String> content() {
			return doGetObservableFluxChatResponse(this.request).map(r -> {
				...
				if(r.getResult().getOutput().getText() == null && r.getResult().getOutput().getReasonerContent() ==null){
					return "";
				}
				if(r.getResult().getOutput().getReasonerContent() !=null){
					var reasonerContent=r.getResult().getOutput().getReasonerContent();
					if (reasonerContent.equals("<think>")) {//deep_seek_maigc_words
						return String.format("{\"reasoner\":\"%s\"}","");
					}
					return String.format("{\"reasoner\":\"%s\"}",r.getResult().getOutput().getReasonerContent());
				}
				return String.format("{\"content\":\"%s\"}",r.getResult().getOutput().getText());
